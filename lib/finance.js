...
// On the entity tabs (Wendal Inc./RIA/IJT/Admin) the "YTD thru June 2026" period also renders the
// 2026 Forecast — What If table underneath, so its button is labelled differently there than on
// the Board Summary tab, which has no forecast section. Keyed by period key; any period not
// listed here just uses its normal PERIODS label.
export const ENTITY_PERIOD_LABELS = {
  ytd_prior: 'YTD thru June 2026 + Forecast',
};
...
// Full set of months with real data.
export const ALL_MONTHS = ['January', 'February', 'March', 'April', 'May', 'June', 'July'];
// Trend charts stop at the last fully-completed month (excludes the current partial month,
// e.g. July 1-15) so the last point on the chart isn't misleadingly low.
export const TREND_MONTHS = ALL_MONTHS.slice(0, -1);
...
// Number of forecast months the flat monthly rate applies to (July through December).
export const FORECAST_MONTH_COUNT = 6;
...
export function buildForecastRows(forecastRows, forecastRules, overrides, classKeys) {
  const combined = {};
  ...
  Object.values(combined).forEach((row) => {
    ...
    const ytdActualPortion = row.forecastTotal - row.monthlyForecast * FORECAST_MONTH_COUNT;
    const total = ytdActualPortion + monthly * FORECAST_MONTH_COUNT;
  ...
  Object.values(combined).forEach((row) => {
    ...
    out.push({
      ...row,
      method: '',
      monthlyForecast: row.monthlyForecast + bump / FORECAST_MONTH_COUNT,
