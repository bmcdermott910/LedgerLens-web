...
import { combinedRows, computeBuckets, BUCKET_ROWS, combineWagesByPerson } from './finance';
import { buildPeriodModel, resolvePeriod } from './periods';
import {
  fetchGlRows, fetchTransactions, fetchWagesByPerson, fetchEmployeeBudgets, fetchMonths,
  fetchForecastMeta,
} from './queries';
...
// Periods are derived from the `months` table rather than a constant, so a lookup has to load
// the calendar before it can resolve a period key. Cached for the life of one request so a
// multi-step answer doesn't re-query the calendar for every tool call.
let periodModelPromise = null;
export function loadPeriodModel() {
  if (!periodModelPromise) {
    periodModelPromise = Promise.all([fetchMonths(), fetchForecastMeta()]).then(([m, meta]) =>
      buildPeriodModel(m, meta)
    );
  }
  return periodModelPromise;
}
export function resetPeriodModelCache() {
  periodModelPromise = null;
}

async function periodFor(periodKey) {
  const model = await loadPeriodModel();
  const p = resolvePeriod(model, periodKey);
...
export async function bucketVariance({ entity, period }) {
  ...
  const p = await periodFor(period);
...
// Built per request because the available periods depend on which months are loaded.
export function buildToolSchemas(model) {
  const periodProp = {
    type: 'string',
    enum: model.periods.map((p) => p.key),
    description: `Reporting period. Available: ${model.periods
      .map((p) => `${p.key} = ${p.label}`)
      .join('; ')}. Default to the period the user is currently viewing.`,
  };
  return schemaTemplates(entityProp, periodProp);
}

function schemaTemplates(entityProp, periodProp) {
  return [
  {
